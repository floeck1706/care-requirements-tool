  ...
  (function($, window, document) {
    ...
    defaults = {
      ...
      formatChoice: function(choice, query) {
        ...
        if (i >= 0) {
		  ...
		  if(head===undefined) head="<leer>"; else {};
          var text = "" + head + "<span class='highlight'>" + matchedText + "</span>" + (tail.join(matchedText));
    ...
    ImmyBox = (function() {
      function ImmyBox(element, options) {
		...
		//this.element.on('blur', function(){self.selectHighlightedChoice();self.hideResults();});
      }
      ...
      ImmyBox.prototype.insertFilteredChoiceElements = function(query) {
        ...
        results = truncatedChoices.map(function(choice) {
          var li;
          li = $("<li class='" + pluginName + "_choice " + choice.type+"'></li>");
          li.attr('data-value', choice.value);
		  var icon;
		  if(choice.icon!=null) {icon = "<i class='"+choice.icon+"'/>&#160;";} else icon = "";
          li.html(icon + format(choice, query));
        ...
        if (results.length === 0) {
          info = $("<p class='" + pluginName + "_noresults'>keine Ergebnisse</p>");
          //this.queryResultArea.empty().append(info);
          this.queryResultArea.empty();
        } else {
          if (!selectedOne) {
